...
exports.BinArrayBitmapExtension = void 0;
...
class BinArrayBitmapExtension {
    ...
    static async fetch(rpc, address, programId = programId_1.PROGRAM_ID) {
        ...
        if (info.programAddress !== programId) {
            throw new Error(`BinArrayBitmapExtensionFields account ${address} belongs to wrong program ${info.programAddress}, expected ${programId}`);
        }
    ...
    static async fetchMultiple(rpc, addresses, programId = programId_1.PROGRAM_ID) {
        ...
        return infos.map((info) => {
            ...
            if (info.programAddress !== programId) {
                throw new Error(`BinArrayBitmapExtensionFields account ${info.address} belongs to wrong program ${info.programAddress}, expected ${programId}`);
            }
            return this.decode(Buffer.from(info.data));
        });
    }
    ...
    toJSON() {
        return {
            lbPair: this.lbPair,
            positiveBinArrayBitmap: this.positiveBinArrayBitmap.map((item) => item.map((item) => item.toString())),
            negativeBinArrayBitmap: this.negativeBinArrayBitmap.map((item) => item.map((item) => item.toString())),
        };
    }
    static fromJSON(obj) {
        return new BinArrayBitmapExtension({
            lbPair: (0, kit_1.address)(obj.lbPair),
            positiveBinArrayBitmap: obj.positiveBinArrayBitmap.map((item) => item.map((item) => new bn_js_1.default(item))),
            negativeBinArrayBitmap: obj.negativeBinArrayBitmap.map((item) => item.map((item) => new bn_js_1.default(item))),
        });
    }
}
exports.BinArrayBitmapExtension = BinArrayBitmapExtension;
//# sourceMappingURL=BinArrayBitmapExtension.js.map