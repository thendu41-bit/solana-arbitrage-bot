...
exports.fetchStakePoolMev = exports.fetchStakeAccount = exports.fetchStakePoolActiveStates = exports.fetchNativeStakeAccounts = void 0;
 ...
 */
const fetchNativeStakeAccounts = async (connection, publicKey, opts = {
    filterInactive: true,
}) => {
    ...
    try {
        ...
        const accounts = await connection.getParsedProgramAccounts(web3_js_1.StakeProgram.programId, {
            filters: [
                {
                    memcmp: {
                        offset: 12,
            ...
            if (!stakeInfo.stake?.delegation ||
                (opts.filterInactive &&
                    (Number(stakeInfo.stake.delegation.activationEpoch) >= epochInfo.epoch ||
                        stakeInfo.stake.delegation.deactivationEpoch !== mrgn_common_1.MAX_U64))) {
                return;
            }
            const validatorAddress = stakeInfo.stake.delegation.voter;
            const accountPubkey = acc.pubkey;
            const amount = Number(stakeInfo.stake.delegation.stake) / web3_js_1.LAMPORTS_PER_SOL;
        ...
        return Promise.all(Array.from(validatorMap.entries()).map(async ([validatorAddress, accounts]) => {
            const poolKey = (0, single_spl_pool_1.findPoolAddress)(new web3_js_1.PublicKey(validatorAddress));
            const poolMintKey = (0, single_spl_pool_1.findPoolMintAddress)(poolKey);
            ...
            return {
                validator: new web3_js_1.PublicKey(validatorAddress),
                poolKey,
                poolMintKey,
 ...
 */
const fetchStakePoolActiveStates = async (connection, validatorVoteAccounts) => {
    ...
    validatorVoteAccounts.forEach((validatorVoteAccount) => {
        const poolAddress = (0, single_spl_pool_1.findPoolAddress)(validatorVoteAccount);
        const poolStakeAddress = (0, single_spl_pool_1.findPoolStakeAddress)(poolAddress);
        const poolMintAddress = (0, single_spl_pool_1.findPoolMintAddress)(poolAddress);
    ...
    const poolStakeAccounts = Object.fromEntries((await (0, mrgn_common_1.chunkedGetRawMultipleAccountInfoOrdered)(connection, poolStakeAddressKeys)).map((ai, index) => [
        poolStakeAddressRecord[poolStakeAddressKeys[index]],
        ai?.data || null,
    ]));
    validatorVoteAccounts.map((validatorVoteAccount) => {
        const stakeAccount = (0, exports.fetchStakeAccount)(poolStakeAccounts[validatorVoteAccount.toBase58()]);
...
//   return rates;
// };
const fetchStakeAccount = function (data) {
    let offset = 0;
...
const fetchStakePoolMev = async (connection, validatorVoteAccounts) => {
    ...
    return (0, mrgn_common_1.chunkedGetRawMultipleAccountInfoOrdered)(connection, allAddresses).then((accountInfos) => {
        ...
        validatorVoteAccounts.forEach((validatorVoteAccount, index) => {
            ...
            if (poolStakeInfo && onRampInfo) {
                const stakeDecoded = (0, exports.fetchStakeAccount)(poolStakeInfo.data);
                const onrampDecoded = (0, exports.fetchStakeAccount)(onRampInfo.data);
