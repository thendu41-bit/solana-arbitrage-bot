...
async function simulateAccountHealthCache(props) {
    ...
    if (crankPythIxs.postInstructions.length > 0) {
        txs.push(...(0, mrgn_common_1.splitInstructionsToFitTransactions)([computeIx], [
            fundAccountIx,
    ...
    const swbTx = new web3_js_1.VersionedTransaction(messageV0);
    txs.push(swbTx);
    const healthTx = new web3_js_1.TransactionMessage({
        payerKey: program.provider.publicKey,
        recentBlockhash: blockhash,
        instructions: [...healthPulseIxs.instructions],
    }).compileToV0Message([]);
    const healthTxV0 = new web3_js_1.VersionedTransaction(healthTx);
    txs.push(healthTxV0);
    if (txs.length > 5) {
...
async function createUpdateFeedIx(props) {
    ...
    const pullFeedInstances = props.swbPullOracles.map((pubkey) => new on_demand_1.PullFeed(swbProgram, pubkey));
